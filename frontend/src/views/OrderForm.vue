...
const submit = async () => {
  ...
  if (!state.form.name?.trim()) {
    ...
    if (!state.form.cardNumber?.trim()) {
      ...
      return;
    } else if (state.form.cardNumber.length > 16 || parseInt(state.form.cardNumber).toString()
        !== state.form.cardNumber) {
      window.alert("카드 번호는 16자리여야 합니다.");
      document.getElementById("cardNum")?.focus();
      return;
    }
  }
  ...
  <form class="order-form" @submit.prevent="submit">
    <div class="container">
      <div class="py-5 text-center">
        ...
        <p class="h6 font-lg mt-3">주문 내역을 확인하시고 주만하기 버튼을 클릭해주세요.</p>
      </div>
      <div class="row g-5">
          ...
          <div class="my-3">
            <div class="form-check">
              <input id="card" name="paymentMethod" type="radio" class="form-check-input"
                     value="card" v-model="state.form.payment" checked>
